...
const Pay = () => {
  ...
  const handleOrdered=()=>
  {
      axios.post(`https://sapdealbackend-1.onrender.com/orders/done`,{product:cart},{headers:{token:localStorage.getItem("token")}}).then(r=>
      {
        if(r.data.msg)
        {
          toast({
          ...
          cart.map((ele)=>
          {
            axios.patch("https://sapdealbackend-1.onrender.com/carts/delete",{productId:ele._id},{headers:{token:localStorage.getItem("token")}})
          })
  ...
  return (
    <div>
        <Box  backgroundColor="gray.50">
         ...
         <Button  color="white" backgroundColor="#E40046" mt={8} onClick={handleOrdered}>PAY RS. {cart.reduce((c,el)=>el.product != null && c+(el?.product?.price*el?.quntity),0)}</Button>
