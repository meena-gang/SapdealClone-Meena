...
const AdminHome = () => {
    ...
    const handleSearch=()=>
    {
      ...
      setSearchParams({search:search,page:page})
       axios.get(`https://sapdealbackend-1.onrender.com/products/?search=${search}&page=${page}`,{headers:{token:localStorage.getItem("token")}})
    ...
    useEffect(()=>
    {
      ...
      setSearchParams({search:search,page:page})
      axios.get(`https://sapdealbackend-1.onrender.com/products/?search=${search}&page=${page}`,{headers:{token:localStorage.getItem("token")}}).then(r=>setData(r.data));
    },[random,page]);
   
    const handleDelete=(item)=>
    {
      axios.delete(`https://sapdealbackend-1.onrender.com/products/deleteProduct/${item._id}`,{headers:{token:localStorage.getItem("token")}}).then(r=>
      {
        if(r.data.msg)
        {
            toast({
